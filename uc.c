...
{
	bool res = uc->read_virtual_mem(uc->cpu, address, _bytes, size);
	if (res) return UC_ERR_OK;
	else return UC_ERR_MAP;
	
}
UNICORN_EXPORT
uc_err uc_virtual_mem_write(uc_engine *uc, uint64_t address, const void *_bytes, size_t size)
{
	bool res = uc->write_virtual_mem(uc->cpu, address, _bytes, size);
	if (res) return UC_ERR_OK;
	else return UC_ERR_MAP;
}
